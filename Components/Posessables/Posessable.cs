    ...
    public class Posessable : IPosessable
    {
        ...
        private void HandleInput(float seconds)
        {
            ...
            else
            {
                ...
                {
                    PlayerState.MovementX = PlayerState.MovementStateX.Slowing;
                }
                else
                {
        ...
        private void HandleState(float seconds)
        {
            ...
            if (Math.Abs(Velocity.X) > Globals.MaxSpeed.X)
            {
                Velocity = new Vector2(Math.Sign(Velocity.X) * Globals.MaxSpeed.X, Velocity.Y);
            }

            if (Math.Abs(Velocity.Y) > Globals.MaxSpeed.Y)
            {
                Velocity = new Vector2(Math.Sign(Velocity.X) * Globals.MaxSpeed.X, Math.Sign(Velocity.Y) * Globals.MaxSpeed.Y);
            }
